"""Local TCP JSON-RPC client for LinknLink infrared remotes."""

from __future__ import annotations

import asyncio
import base64
import binascii
import json
import struct
from contextlib import suppress
from typing import Any

from .devices import DeviceCapability
from .models import UltraDevice

DEFAULT_REMOTE_PORT = 502
DEFAULT_REMOTE_TIMEOUT = 5.0
DEFAULT_LEARN_TIMEOUT = 30.0
DEFAULT_LEARN_POLL_INTERVAL = 1.0
MAX_RESPONSE_SIZE = 16 * 1024
IR_FRAME_HEADER_SIZE = 4
IR_FREQUENCY_KHZ = 38
MAX_REQUEST_ID = 0x7FFFFFFF


class LinknLinkRemoteError(Exception):
    """Base error for the local infrared remote protocol."""


class LinknLinkRemoteConnectionError(LinknLinkRemoteError):
    """The remote TCP endpoint could not be reached or disconnected."""


class LinknLinkRemoteProtocolError(LinknLinkRemoteError):
    """The remote returned an invalid JSON-RPC response."""


class LinknLinkRemoteCommandError(LinknLinkRemoteError):
    """The remote rejected a valid JSON-RPC request."""

    def __init__(self, message: str, *, code: int | None = None) -> None:
        super().__init__(message)
        self.code = code


class LinknLinkRemoteTimeoutError(LinknLinkRemoteConnectionError):
    """A remote request or infrared learning operation timed out."""


class LinknLinkRemoteClient:
    """Serialize JSON-RPC operations over the device's single TCP connection."""

    def __init__(
        self,
        device: UltraDevice,
        *,
        port: int = DEFAULT_REMOTE_PORT,
        timeout: float = DEFAULT_REMOTE_TIMEOUT,
        max_response_size: int = MAX_RESPONSE_SIZE,
    ) -> None:
        if DeviceCapability.REMOTE not in device.capabilities:
            raise ValueError("device does not support the local remote protocol")
        if not device.ip.strip():
            raise ValueError("device IP address is required")
        if isinstance(port, bool) or not 1 <= port <= 0xFFFF:
            raise ValueError("remote port must be between 1 and 65535")
        if timeout <= 0:
            raise ValueError("remote timeout must be greater than zero")
        if max_response_size <= 0:
            raise ValueError("maximum response size must be greater than zero")

        self.device = device
        self.port = port
        self.timeout = timeout
        self.max_response_size = max_response_size
        self._lock = asyncio.Lock()
        self._reader: asyncio.StreamReader | None = None
        self._writer: asyncio.StreamWriter | None = None
        self._request_id = 0

    @property
    def connected(self) -> bool:
        """Return whether a usable TCP writer is currently open."""
        return self._writer is not None and not self._writer.is_closing()

    async def close(self) -> None:
        """Close the persistent connection after any active request completes."""
        async with self._lock:
            await self._close_locked()

    async def start_learning(self) -> None:
        """Put the infrared receiver into learning mode."""
        await self._study(True)

    async def stop_learning(self) -> None:
        """Leave infrared learning mode."""
        await self._study(False)

    async def read_learned_code(self) -> str | None:
        """Read the learned Base64 frame, or return None while no code is ready."""
        async with self._lock:
            return await self._read_learned_code_locked()

    async def send_code(self, data: str) -> None:
        """Transmit one validated Base64 infrared frame."""
        _validate_ir_code(data)
        async with self._lock:
            result = await self._request_locked("irdaSend", {"data": data})
            _require_success_result(result, "infrared send")

    async def learn_code(
        self,
        *,
        timeout: float = DEFAULT_LEARN_TIMEOUT,
        poll_interval: float = DEFAULT_LEARN_POLL_INTERVAL,
    ) -> str:
        """Learn one infrared frame and always stop learning before returning."""
        if timeout <= 0:
            raise ValueError("learn timeout must be greater than zero")
        if poll_interval <= 0:
            raise ValueError("learn poll interval must be greater than zero")

        async with self._lock:
            result = await self._request_locked("irdaStudy", {"start": 1})
            _require_success_result(result, "start learning")
            deadline = asyncio.get_running_loop().time() + timeout
            primary_error: BaseException | None = None
            try:
                while True:
                    remaining = deadline - asyncio.get_running_loop().time()
                    if remaining <= 0:
                        raise LinknLinkRemoteTimeoutError("infrared learning timed out")
                    if code := await self._read_learned_code_locked():
                        return code
                    remaining = deadline - asyncio.get_running_loop().time()
                    if remaining <= 0:
                        raise LinknLinkRemoteTimeoutError("infrared learning timed out")
                    await asyncio.sleep(min(poll_interval, remaining))
            except BaseException as err:
                primary_error = err
                raise
            finally:
                try:
                    result = await self._request_locked("irdaStudy", {"start": 0})
                    _require_success_result(result, "stop learning")
                except LinknLinkRemoteError as err:
                    if primary_error is None:
                        raise
                    primary_error.add_note(f"stopping infrared learning also failed: {err}")

    async def _study(self, start: bool) -> None:
        async with self._lock:
            result = await self._request_locked("irdaStudy", {"start": int(start)})
            _require_success_result(result, "start learning" if start else "stop learning")

    async def _read_learned_code_locked(self) -> str | None:
        result = await self._request_locked("irdaRead")
        if not isinstance(result, dict) or set(result) != {"data"}:
            raise LinknLinkRemoteProtocolError("invalid infrared read result")
        data = result["data"]
        if not isinstance(data, str):
            raise LinknLinkRemoteProtocolError("invalid infrared read data")
        if data == "NULL":
            return None
        try:
            _validate_ir_code(data)
        except ValueError as err:
            raise LinknLinkRemoteProtocolError(f"invalid learned infrared code: {err}") from err
        return data

    async def _request_locked(
        self,
        method: str,
        params: dict[str, object] | None = None,
    ) -> object:
        request_id = self._next_request_id()
        request: dict[str, object] = {
            "jsonrpc": "2.0",
            "method": method,
            "id": request_id,
        }
        if params is not None:
            request["params"] = params
        payload = json.dumps(request, separators=(",", ":")).encode()

        try:
            async with asyncio.timeout(self.timeout):
                await self._ensure_connected_locked()
                assert self._writer is not None
                self._writer.write(payload)
                await self._writer.drain()
                response = await self._read_response_locked()
        except TimeoutError as err:
            await self._close_locked()
            raise LinknLinkRemoteTimeoutError(f"remote request {method} timed out") from err
        except LinknLinkRemoteError:
            await self._close_locked()
            raise
        except (OSError, ConnectionError) as err:
            await self._close_locked()
            raise LinknLinkRemoteConnectionError(
                f"remote request {method} failed: {str(err) or type(err).__name__}"
            ) from err

        try:
            return _parse_rpc_response(response, request_id)
        except LinknLinkRemoteProtocolError:
            await self._close_locked()
            raise

    async def _ensure_connected_locked(self) -> None:
        if self.connected:
            return
        await self._close_locked()
        try:
            self._reader, self._writer = await asyncio.open_connection(
                self.device.ip,
                self.port,
            )
        except (OSError, ConnectionError) as err:
            raise LinknLinkRemoteConnectionError(
                f"could not connect to remote at {self.device.ip}:{self.port}: {str(err) or type(err).__name__}"
            ) from err

    async def _read_response_locked(self) -> dict[str, Any]:
        assert self._reader is not None
        payload = bytearray()
        decoder = json.JSONDecoder()
        while len(payload) <= self.max_response_size:
            chunk = await self._reader.read(min(4096, self.max_response_size + 1 - len(payload)))
            if not chunk:
                raise LinknLinkRemoteConnectionError("remote closed the connection before responding")
            payload.extend(chunk)
            try:
                text = payload.decode()
            except UnicodeDecodeError as err:
                if err.reason == "unexpected end of data" and len(payload) <= self.max_response_size:
                    continue
                raise LinknLinkRemoteProtocolError("remote response is not valid UTF-8") from err
            try:
                value, end = decoder.raw_decode(text.lstrip())
            except json.JSONDecodeError:
                continue
            if text.lstrip()[end:].strip():
                raise LinknLinkRemoteProtocolError("remote response contains trailing data")
            if not isinstance(value, dict):
                raise LinknLinkRemoteProtocolError("remote response is not a JSON object")
            return value
        raise LinknLinkRemoteProtocolError("remote response exceeds the size limit")

    async def _close_locked(self) -> None:
        writer = self._writer
        self._reader = None
        self._writer = None
        if writer is None:
            return
        writer.close()
        with suppress(OSError, ConnectionError):
            await writer.wait_closed()

    def _next_request_id(self) -> int:
        self._request_id = self._request_id % MAX_REQUEST_ID + 1
        return self._request_id


def _parse_rpc_response(response: dict[str, Any], request_id: int) -> object:
    if response.get("jsonrpc") != "2.0":
        raise LinknLinkRemoteProtocolError("invalid JSON-RPC response version")
    response_id = response.get("id")
    if isinstance(response_id, bool) or response_id != request_id:
        raise LinknLinkRemoteProtocolError(f"JSON-RPC response ID mismatch ({response_id!r}, expected {request_id})")
    has_result = "result" in response
    has_error = "error" in response
    if has_result == has_error:
        raise LinknLinkRemoteProtocolError("JSON-RPC response must contain exactly one result or error")
    if has_error:
        error = response["error"]
        if not isinstance(error, dict):
            raise LinknLinkRemoteProtocolError("invalid JSON-RPC error object")
        code = error.get("code")
        message = error.get("message")
        if isinstance(code, bool) or not isinstance(code, int) or not isinstance(message, str):
            raise LinknLinkRemoteProtocolError("invalid JSON-RPC error fields")
        raise LinknLinkRemoteCommandError(f"remote command failed: {message}", code=code)
    return response["result"]


def _require_success_result(result: object, operation: str) -> None:
    if isinstance(result, bool) or not isinstance(result, int):
        raise LinknLinkRemoteProtocolError(f"invalid {operation} result")
    if result != 0:
        raise LinknLinkRemoteCommandError(f"{operation} failed with result {result}")


def _validate_ir_code(data: str) -> None:
    if not isinstance(data, str) or not data:
        raise ValueError("infrared code must be a non-empty Base64 string")
    try:
        frame = base64.b64decode(data, validate=True)
    except (ValueError, binascii.Error) as err:
        raise ValueError("infrared code is not valid Base64") from err
    if len(frame) < IR_FRAME_HEADER_SIZE:
        raise ValueError("infrared code frame is too short")
    if frame[0] != IR_FREQUENCY_KHZ:
        raise ValueError("infrared code frequency must be 38 kHz")
    data_length = struct.unpack_from("<H", frame, 2)[0]
    if data_length != len(frame) - IR_FRAME_HEADER_SIZE:
        raise ValueError("infrared code frame length does not match its payload")
